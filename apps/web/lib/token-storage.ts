 ...
 * talks straight to the API service over NEXT_PUBLIC_API_URL, so there is no
 * same-origin route handler to set/read a cookie from. localStorage is the
 * pragmatic choice for this issue, with the known risk that any XSS on this
 * origin can read both tokens. Follow-up work should introduce a Next.js
 * route handler proxy that sets an httpOnly, SameSite=strict cookie and have
 * middleware gate routes off of it instead of this client-side check.
 */

const ACCESS_TOKEN_KEY = "echoflow.access_token";
const REFRESH_TOKEN_KEY = "echoflow.refresh_token";
const USER_KEY = "echoflow.user";
