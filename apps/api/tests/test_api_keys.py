...
require_api_key is exercised directly here as a dependency function against
real, committed database state -- the same database the HTTP-level tests
write through, just without an HTTP layer in front of it (no data route
requiring it exists yet in this milestone).
"""
...
async def test_create_api_key_returns_raw_key_once(client: AsyncClient) -> None:
    ...
    assert body["raw_key"].startswith("efl_live_")
    assert body["prefix"].startswith("efl_live_")
...
async def test_require_api_key_rejects_unknown_key(db_session: AsyncSession) -> None:
    with pytest.raises(HTTPException) as exc_info:
        await require_api_key(
            authorization="Bearer efl_live_totally-made-up", session=db_session
        )
...
async def test_cannot_revoke_another_organizations_api_key(client: AsyncClient) -> None:
    ...
    # Org B attempting to revoke org A's key must not succeed. 404, not 403,
    # so org B cannot distinguish "not found" from "not yours" and probe for
    # other orgs' key ids.
    revoke_response = await client.delete(
...
async def test_revoking_unknown_api_key_returns_404(client: AsyncClient) -> None:
    access_token, _ = await _register_and_get_access_token(client)

    response = await client.delete(
        f"/v1/api-keys/{uuid.uuid4()}", headers={"Authorization": f"Bearer {access_token}"}
    )
    assert response.status_code == 404


async def test_revoking_already_revoked_api_key_is_a_no_op(client: AsyncClient) -> None:
    access_token, _ = await _register_and_get_access_token(client)
    create_response = await client.post(
        "/v1/api-keys", headers={"Authorization": f"Bearer {access_token}"}
    )
    ...
    first_revoke = await client.delete(
        f"/v1/api-keys/{key_id}", headers={"Authorization": f"Bearer {access_token}"}
    )
