...
Same dogfooded design as the Helios and Nexus projects (see docs/TDD.md
section 8): a log record factory injects the current correlation id onto
every ``LogRecord`` before it reaches any handler, so the JSON formatter
only has to read ``record.correlation_id`` off the record rather than the
id being threaded through every function call as an explicit parameter.
...
def set_correlation_id(value: str) -> contextvars.Token[str | None]:
    """Set the correlation id for the current request or job.

    Thin, named wrapper around ``correlation_id_var.set`` so callers outside
    this module (``CorrelationIdMiddleware``, the worker, and any future
    route handler that needs to replace the generic id with something more
    specific, e.g. a webhook route tagging a request with the Twilio
    ``CallSid``/``MessageSid`` once the payload has been parsed) don't need
    to import and poke the contextvar directly. Returns the token
    ``correlation_id_var.reset`` needs to restore whatever value was set
    before this call.
    """
