"""FastAPI dependencies that authenticate a request under one of EchoFlow's
two auth schemes (see docs/TDD.md section 3.6):

- ``require_api_key``: ``Authorization: Bearer efl_live_...`` -> ``Organization``.
...
async def require_api_key(
    ...
    """Resolve ``Authorization: Bearer efl_live_...`` to the owning, non-revoked
