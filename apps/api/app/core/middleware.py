...
class CorrelationIdMiddleware:
    ...
    leaks into whatever request this worker handles next.

    A route handler can replace this generic id later in the same request
    via ``set_correlation_id`` (e.g. a webhook route overriding it with the
    Twilio ``CallSid``/``MessageSid`` once the payload is parsed); the
    response header keeps the id this middleware generated at request start
    either way, since that is set before the handler runs.
